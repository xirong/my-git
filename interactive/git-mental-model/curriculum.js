    ...
    recovery: {
      ...
      states(choice) {
        ...
        const copy = [
          ...
          { caption: rescue ? ['创建 recovered -> C2 后，C2 从 branch 起点重新可达。', 'After recovered -> C2 is created, C2 is reachable again from a branch.'] : ['同样为 C2 建立 recovered 并验证它存在，作为对照；C3 仍不获得任何 branch，保留只依赖 reflog 与过期策略。', 'Create recovered for C2 here as well and verify that it exists, as the control; C3 still gets no branch, so its retention depends only on reflog and expiration policy.'], command: rescue ? 'git branch recovered <C2-id>' : 'git branch recovered <C2-id>\ngit show-ref --verify refs/heads/recovered', output: 'refs/heads/recovered -> C2' },
          ...
          { caption: rescue ? ['C2 仍可读，因为 recovered 使它可达。C3 已无法解析。', 'C2 remains readable because recovered makes it reachable. C3 no longer resolves.'] : ['recovered 让 C2 在清理后仍可读；没有 ref 或 reflog 保护的 C3 已无法解析。未 add 或 commit 的编辑不在这个恢复承诺内。', 'recovered keeps C2 readable after cleanup, while C3 had no ref or reflog protection and no longer resolves. Unstaged, uncommitted edits are outside this recovery promise.'], command: 'git cat-file -e recovered^{commit}\ngit cat-file -e <C3-id>^{commit} || printf "C3: missing\\n"', output: rescue ? 'C2: readable\nC3: missing' : 'C2: readable\nfatal: Not a valid object name <C3-id>\nC3: missing' }
        ];
    ...
    storage: {
      ...
      states(choice) {
        ...
        const captions = prune ? [
          ...
          ['O 没有可达路径并被移除；main 继续保护 C，这里对同一个 C 再次保存 git show 输出。', 'O has no reachable path and is removed; main continues to protect C, and git show output for the same C is captured again here.'],
          ...
          ['维护后再次保存同一个 C 的 git show 输出并 diff：物理文件已变化，git show C 仍逐字节一致。这里不声称固定体积或性能提升。', 'Capture git show for the same C again after maintenance and diff it: physical files changed while git show C stayed byte-for-byte identical. This makes no fixed size or performance claim.']
        ];
        return captions.map((caption, step) => {
          ...
          return { caption, command: prune ? ['git show <C-id>','git show <C-id> > show-before.txt\ngit cat-file -e <C-id>\ngit cat-file -e <O-id>','git prune --expire=now','git cat-file -e <O-id> || printf "O: missing\\n"\ngit show <C-id> > show-after.txt','diff -u show-before.txt show-after.txt'][step] : ['git count-objects -vH','git show <C-id> > show-before.txt','git repack -ad','git commit-graph write --reachable\ngit gc --no-prune','git show <C-id> > show-after.txt\ndiff -u show-before.txt show-after.txt'][step], output: step === 4 ? 'no diff for stable commit C' : removed ? 'O: missing\nC: readable' : 'state shown above', nodes, edges: [edge('main','c'),edge('c','tree'),edge('tree','blob')], facts: [['git show C', 'unchanged'], ['physical structure', prune ? removed ? 'orphan removed' : 'orphan present' : changed ? 'pack / graph may change' : 'observed only'], ['performance', 'not claimed']] };
        });
      }
    }
  };
